...
if [[ ! ${BORG_REPO} ]]; then
	printf "\nPlease set BORG_REPO\n"
	exit 1
fi

if [[ ! ${BORG_S3_BACKUP_ORIGIN} ]]; then
	printf "\nPlease set BORG_S3_BACKUP_ORIGIN\n"
	exit 1
fi

if [[ ! ${BORG_S3_BACKUP_BUCKET} ]]; then
	printf "\nPlease set BORG_S3_BACKUP_BUCKET\n"
	exit 1
fi

if [[ ! ${BORG_S3_BACKUP_AWS_PROFILE} ]]; then
	printf "\nPlease set BORG_S3_BACKUP_AWS_PROFILE (awscli profile)\n"
	exit 1
fi
...
if [[ ${OPERATION_STATUS} == 0 ]]; then
	...
	borg prune --keep-daily=7 --keep-weekly=4 --keep-monthly=12 --keep-yearly=10 --save-space --error "${BORG_REPO}"

	# Sync borg repo to s3
	printf "\n\nStarting sync ${BORG_REPO} to S3...\n"
...
if hash notify-send 2>/dev/null; then
	if [[ ${OPERATION_STATUS} == 0 ]]; then
		notify-send "Home folder backup" "${STATUS_MESSAGE}" --urgency=normal --icon=dialog-information
	else
		notify-send "Home folder backup" "${STATUS_MESSAGE}" --urgency=critical --icon=dialog-error
	fi
fi
